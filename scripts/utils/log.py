"""
В данный модуль вынесен функционал, связанный с логировнием
"""
import json
import logging
...
def get_log_file_name():
    ...
    return "log_from_" + current_date + ".log"
...
class HDFSHandler(Handler):
    def __init__(self, hdfs_directory_path: str):
        super().__init__()
        self.hdfs_file_path = hdfs_directory_path + '/' + get_log_file_name()
    ...
    def emit(self, record):
        ...
        try:
            ...
            if not response.ok:
                ...
                rs = response.reason
                self.logger.warning("Logger response status code {}: {}".format(sc, rs))
    ...
    def emit(self, record):
        ...
        try:
            ...
            if not response.ok:
                sc = response.status_code
                rs = response.reason
                logging.warning("Logger response status code {}: {}".format(sc, rs))
...
class CTLLogFormatter(Formatter):
    def __init__(self, loading_id: int, ctl_date_fmt='%Y-%m-%d %H:%M:%S'):
        super().__init__()
...
def exception_hook(exc_type, exc_value, exc_traceback):
    ...
    logging.error(msg="{}\n{}".format(exc_value, fmt_stacktrace),
